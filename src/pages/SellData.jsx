...
async function getData () {
  ...
  const res = await db.collection('Test').where("onSale","==",true).get()
  console.log(res.data)
}
async function sendData (Title, description, FileSize, DealAddress, SalePrice, cid) {
  await db.collection('SellerData').create([Title, description, FileSize, DealAddress, SalePrice, cid]);
}
...
const SellData = () => {
  ...
  const submitHandler = async event => {
    ...
    await payWithMetamask('0x72d2F62A93305752CC57D48Ea217CA687EA43dc0', '1');

    // *TODO* loading (creating deal on FVM)
    
    // -- store details on polybase for marketplace reference 
    await sendData(enteredTitle, enteredDescription, res.fileSize, 'deALAddr',50, res.cid);
  ...
  const payWithMetamask = async(receiver, strEther)=> {
    ...
    console.log(`payWithMetamask(receiver=${receiver}, sender=${signer}, strEther=${strEther})`)
